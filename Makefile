...
up-db-services:
	...
	@$(DOCKER_COMPOSE) -f docker-compose.yml up -d $(KC_SERVICE_NAME) $(ADMIN_SERVICE_DB_NAME) $(THERAPIST_SERVICE_DB_NAME) $(PATIENT_SERVICE_DB_NAME) $(VNPATIENT_SERVICE_DB_NAME) $(PHONE_SERVICE_DB_NAME) $(OPEN_LIBRARY_SERVICE_DB_NAME) $(MONGO_SERVICE_NAME)
...
restore-keycloak-db:
	...
	@if [ -f "./config/db_dump/kc_db_dump.sql.gz" ]; then \
		gunzip < ./config/db_dump/kc_db_dump.sql.gz | @$(DOCKER_COMPOSE) -f docker-compose.yml exec -T $(KC_SERVICE_NAME) sh -c 'exec mysql -uroot -p$(DB_ROOT_PASS) $(KC_DB_NAME)'; \
	else \
...
git-pull:
	...
	git -C ${adminPath} pull || git clone git@git.web-essentials.asia:hiv-tra-20/admin-service.git ${adminPath}
	git -C ${therapistPath} pull || git clone git@git.web-essentials.asia:hiv-tra-20/therapist-service.git ${therapistPath}
	git -C ${patientPath} pull || git clone git@git.web-essentials.asia:hiv-tra-20/patient-service.git ${patientPath}
	git -C ${adminWebPath} pull || git clone git@git.web-essentials.asia:hiv-tra-20/admin-web-app.git ${adminWebPath}
	git -C ${therapistWebPath} pull || git clone git@git.web-essentials.asia:hiv-tra-20/therapist-web-app.git ${therapistWebPath}
	git -C ${keycloakPath} pull || git clone git@git.web-essentials.asia:hiv-tra-20/keycloak.git ${keycloakPath}
	git -C ${krakendPath} pull || git clone git@git.web-essentials.asia:hiv-tra-20/krakend.git ${krakendPath}
	git -C ${openLibraryPath} pull || git clone git@git.web-essentials.asia:hiv-tra-20/hic-service.git ${openLibraryPath}
	git -C ${openLibraryWebPath} pull || git clone git@git.web-essentials.asia:hiv-tra-20/hic-webapp.git ${openLibraryWebPath}
	git -C ${phonePath} pull || git clone git@git.web-essentials.asia:hiv-tra-20/phone-service.git ${phonePath}
